...
void ShowBar(int Percent,int MaxSize,char* Msg)
{
    ...
    putchar('[');
        for(i = (Percent*MaxSize)/1000000; a<i; ++a) putchar('#');
...
int pop(char * parameter)
{
    ...
    fseek(fp, 8 * (hash(parameter)), SEEK_SET);
    fread(&block, 8, 1, fp);
...
void main(){
    ...
    fp = fopen(NAME_FILE, /*"rb+"*/"r+");										//Abre el archivo (dataDogs.dat)
    ...
    if(fp == NULL){													//Si no existe:
        ...
        if(opcion == 'N' || opcion == 'n'){								//En caso negativo, sale del programa.
            ...
            freopen(NAME_FILE, /*"rb+"*/"r+", fp);								//Abre el archivo y lo asigna a el flo de datos fp
            ...
            if(opcion == 'Y' || opcion == 'y' || opcion == 'S' || opcion == 's'){
                ...
                petNames = fopen("nombresMascotas.dat" , /*"rb+"*/ "r+");				//de nombres de perros para la creacion aleatoria de
                													//los registros
...
/*
    char nombre[32];
    FILE *test;
    test = fopen("prueba.txt", "w");
    fseek(fp, 32000, SEEK_SET);

    for(i = 0; i < 1000000; i++)
    {
        fseek(fp, (i * 72) + 32000, SEEK_SET);
        fread(&nombre, 32, 1, fp);
        //printf("%s\t", nombre);
        fprintf(test, "%s%li\t", nombre, hash(nombre));
    }
*/
}