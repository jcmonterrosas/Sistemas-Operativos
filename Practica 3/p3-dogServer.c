...
void main(void){
	...
	if (r != 0)
	{
		perror("\n Error en incializar mutex\n");
		exit(-1);
	}
    //Iniciar Semaforo
    /*
    r = sem_init( &semaphore, 0, 1 );
	if (r != 0 ) 
    { 
        perror("\n Error en incializar semaforo\n"); 
        exit(-1);
    } 
	*/
    //Iniciar Pipe
    /*
    if( pipe( pipeFileDescriptor ) ){
		perror( "\n Error en incializar pipe\n" );
		exit(-1);
	}
    if( write( pipeFileDescriptor[1], pipeToken, 1 ) == -1 ){
		perror( "\n Error en escribir  pipe\n" );
		exit(-1);			
	} 
    */
    //Conexion.
